		...
		if($q_no!="0")// check if Quarter was selected
		{
			...
			if($all!="0")
			{
				if($county_name!="0")
				{
					if($report_type==1)
					{
						$graph_tests=$this->reports_charts_model->year_quarter_county_tests($county_name,$q_no,$the_year,$all);
					}
					else if($report_type==2)
					{
						$graph_tests=$this->reports_charts_model->year_quarter_county_errors($county_name,$q_no,$the_year,$all);
					}
					else if($report_type==0)
					{
						$graph_tests=$this->reports_charts_model->year_quarter_county_all($county_name,$q_no,$the_year,$all);
					}
					else if($report_type==3)
					{
						$graph_tests=$this->reports_charts_model->quarter_tests_less_than350($q_no,$facility,$device,$the_year,$all,$county_name,$login_id);
		...
		if($b_no!="0")// check if the biannual criteria is chosen
		{
			...
			if($all!="0")
			{
				if($county_name!="0")
				{
					if($report_type==1)
					{
						$graph_tests=$this->reports_charts_model->year_bian_county_tests($county_name,$b_no,$the_year,$all);
					}
					else if($report_type==2)
					{
						$graph_tests=$this->reports_charts_model->year_bian_county_errors($county_name,$b_no,$the_year,$all);
					}
					else if($report_type==0)
					{
						$graph_tests=$this->reports_charts_model->year_bian_county_all($county_name,$b_no,$the_year,$all);	
					}
					else if($report_type==3)
					{
						$graph_tests=$this->reports_charts_model->bian_tests_less_than350($b_no,$facility,$device,$the_year,$all,$county_name,$login_id);
		...
		if($year!="0")// check if the yearly criteria is chosen
		{
			...
			if($all!="0")
			{
				if($county_name!="0")
				{
					if($report_type==1)
					{
						$graph_tests=$this->reports_charts_model->yearly_county_tests($county_name,$year,$all);
					}
					else if($report_type==2)
					{
						$graph_tests=$this->reports_charts_model->yearly_county_errors($county_name,$year,$all);
					}
					else if($report_type==0)
					{
						$graph_tests=$this->reports_charts_model->yearly_county_all($county_name,$year,$all);
					}
					else if($report_type==3)
					{
						$graph_tests=$this->reports_charts_model->yearly_tests_less_than350($year,$facility,$device,$all,$county_name,$login_id);
