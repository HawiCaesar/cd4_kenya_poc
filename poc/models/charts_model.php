...
class charts_model extends MY_Model{
	public function yearly_device_test_trends($year,$user_filter_used){
	...
	public function yearly_facility_pima_reporting_rates($year){ 
		...
		$data = array();
		array_push($data, array("seriesname"	=>	"Expected Reporting Pima Devices",
								"color"			=>	"A66EDD",
	...
	private function expected_reporting_dev_array($year){
		...
		for($i=1;$i<=12;$i++){
			...
			$devices_removed_array[$i]['cumulative']	=	1;
			foreach ($devices_removed_assoc as $removed) {
